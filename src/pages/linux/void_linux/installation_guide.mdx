...
Manually partition your disk using `cfdisk`--it's easy to use, set partition table to **GPT**.
...
To modify the partition table type, select the disk and use `fdisk`. Run one of the following commands to create a specific partition table:
...
> After making changes, run the **w** command to write the changes and exit.
...
- **BIOS System**: Don't create any filesystem in it. GRUB will install itself without one.
